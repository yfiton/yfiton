...
public final class BooleanConverter implements Converter<Boolean> {
    ...
    @Override
    public Boolean convert(String parameterName, String parameterValue) throws ConversionException {
        if (!parameterValue.equalsIgnoreCase("true")
                && !parameterValue.equalsIgnoreCase("false")) {
            throw new ConversionException(parameterName, parameterValue, Boolean.class);
        }
